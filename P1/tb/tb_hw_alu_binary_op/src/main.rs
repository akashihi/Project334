...
fn main() -> ! {
    ...
    // Y_OUT
    let mut y_out_0 = gpiob.pb8.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_1 = gpiob.pb9.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_2 = gpiob.pb10.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_3 = gpiob.pb11.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_4 = gpiob.pb12.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_5 = gpiob.pb13.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_6 = gpiob.pb14.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    let mut y_out_7 = gpiob.pb15.into_push_pull_output(&mut gpiob.moder, &mut gpiob.otyper);
    ...
    loop {
        for x_out_value in 0u8..=254 {
            set_bit(x_out_value, 7, &mut x_out_7);
            set_bit(x_out_value, 6, &mut x_out_6);
            set_bit(x_out_value, 5, &mut x_out_5);
            set_bit(x_out_value, 4, &mut x_out_4);
            set_bit(x_out_value, 3, &mut x_out_3);
            set_bit(x_out_value, 2, &mut x_out_2);
            set_bit(x_out_value, 1, &mut x_out_1);
            set_bit(x_out_value, 0, &mut x_out_0);

            for y_out_value in 0u8..=254 {

                set_bit(y_out_value, 7, &mut y_out_7);
                set_bit(y_out_value, 6, &mut y_out_6);
                set_bit(y_out_value, 5, &mut y_out_5);
                set_bit(y_out_value, 4, &mut y_out_4);
                set_bit(y_out_value, 3, &mut y_out_3);
                set_bit(y_out_value, 2, &mut y_out_2);
                set_bit(y_out_value, 1, &mut y_out_1);
                set_bit(y_out_value, 0, &mut y_out_0);

                defmt::info!("X_OUT: {=u8:08b}, Y_OUT: {=u8:08b}", x_out_value, y_out_value);
                ...
                block!(timer.wait()).unwrap();
                let x_in = read_x_in(&x_in_0,&x_in_1,&x_in_2,&x_in_3,&x_in_4,&x_in_5,&x_in_6,&x_in_7);
                alu_pass &= check_x_in(x_in, x_out_value.bitxor(y_out_value));
                defmt::info!("X_IN: {=u8:08b}", x_in);
                alu_not(&mut alu_op0, &mut alu_op1, &mut alu_op2);
                block!(timer.wait()).unwrap();
                let x_in = read_x_in(&x_in_0,&x_in_1,&x_in_2,&x_in_3,&x_in_4,&x_in_5,&x_in_6,&x_in_7);
                alu_pass &= check_x_in(x_in, 0b1111_1111);
                ...
                block!(timer.wait()).unwrap();
                let x_in = read_x_in(&x_in_0,&x_in_1,&x_in_2,&x_in_3,&x_in_4,&x_in_5,&x_in_6,&x_in_7);
                alu_pass &= check_x_in(x_in, x_out_value.bitand(y_out_value));
                defmt::trace!("X_IN: {=u8:08b}", x_in);
                alu_or(&mut alu_op0, &mut alu_op1, &mut alu_op2);
                block!(timer.wait()).unwrap();
                let x_in = read_x_in(&x_in_0,&x_in_1,&x_in_2,&x_in_3,&x_in_4,&x_in_5,&x_in_6,&x_in_7);
                alu_pass &= check_x_in(x_in, x_out_value.bitor(y_out_value));
                defmt::trace!("X_IN: {=u8:08b}", x_in);
                if alu_pass {
